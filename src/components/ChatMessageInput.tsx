...
export const ChatMessageInput = ({disabled, onSend} : Props) => {
  ...
  return (
    <div className={`flex border border-gray-800/50 p-2 rounded-md bg-gray-600/50
    ${disabled && 'opacity-50'}`}>

      <textarea ref={textEl} className='flex-1 border-0 bg-transparent resize-none outline-none
      h-7 max-h-48 overflow-y-auto' placeholder='Digite uma mensagem' value={text} 
      onChange={e => setText(e.target.value)} onKeyUp={handleTextKeyUp} disabled={disabled}>
